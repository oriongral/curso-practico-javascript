...
const salary = mexico.map(function (persona) {
  return persona.salary;
});

const salarySorted = salary.sort(function (salaryA, salaryB) {
  return salaryA - salaryB;
});

const medianSalaryGeneral = medianSalary(salarySorted);

const spliceStart = ((salarySorted.length * 90) / 100);
const spliceCount = salarySorted.length - spliceStart;

const salaryTop10 = salarySorted.splice(
  spliceStart,
  spliceCount,
);
